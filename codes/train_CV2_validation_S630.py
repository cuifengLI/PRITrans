...
def cross_validation(train_esm, train_prot, train_label, valid_esm, valid_prot, valid_label, test_esm, test_prot,
    ...
    print(
        f"\n第{i}次实验样本数量：Training samples: {train_esm.shape[0]},Valid samples: {valid_esm.shape[0]} ,Test samples: {test_esm.shape[0]}")
    ...
    qa_model = get_model()
    valiBestModel = 'F:\\protein_stability\\save_model\\XXXXXX.h5'
    ...
    plt.title(f'TEST{i} S394 181 FORWARD Training And Validation RMSE')
    plt.xlabel('60 Epochs 35 patience')
    plt.ylabel('RMSE')
    plt.legend()
    plt.show()
    ...
    plt.title(f'TEST{i} S394 181 FORWARD Training And Validation LOSS')
    plt.xlabel('60 Epochs 35 patience')
    plt.ylabel('Loss')
    plt.legend()
    plt.show()
    ...
    print(
        f"\n第{i}次验证集结果:Validation Loss: {history_callback.history['val_loss'][-1]:.4f}," + f"Validation RMSE: {history_callback.history['val_root_mean_squared_error'][-1]:.4f}")
    ...
    result_end = fr'F:\\protein_stability\\result\\XXXXXX.csv'
...
def evaluate_regression(test_pred, test_label):
    ...
    print("真实值为：" + str(test_label))
    print("预测值为：" + str(test_pred))
    ...
    print(f"\nTEST {i}-TEST Mean Squared Error (MSE): {mse:.4f}")
    print(f"TEST {i}-TEST Mean Absolute Error (MAE): {mae:.4f}")
    print(f"TEST {i}-TEST RMSE: {rmse:.4f}")
    print(f"TEST {i}-TEST R-squared (R2) Score: {r2:.4f}")
    print(f"TEST {i}-TEST PCC: {pearsonr_corr:.4f}")
    print(f"TEST {i}-TEST P-value: {p_value:.4f}")
...
if __name__ == "__main__":
    ...
    all_esm = np.lib.format.open_memmap('../features_npy/S315_total_181_esm.npy')
    all_prot = np.lib.format.open_memmap('../features_npy/S315_total_181_prot.npy')
    all_label = np.lib.format.open_memmap('../features_npy/S315_total_181_label.npy')

    all_label = all_label.astype(np.float)
    print(all_label.dtype)

    for i in range(1, 21):
        train_ESM, test_ESM, train_Prot, test_Prot, train_Y, test_Y = train_test_split(all_esm, all_prot,
                                                                                       all_label, test_size=0.2,
                                                                                       random_state=42)
