...
def cross_validation(train_esm, train_prot, train_label, valid_esm, valid_prot, valid_label, test_esm, test_prot,
    ...
    print(
        f"\n第{k}fold验证集结果：Validation Loss: {history_callback.history['val_loss'][-1]:.4f}," + f"Validation RMSE: {history_callback.history['val_root_mean_squared_error'][-1]:.4f}")
...
if __name__ == "__main__":
    ...
    all_esm = np.lib.format.open_memmap('../features_npy/S315_181_esm.npy')
    all_prot = np.lib.format.open_memmap('../features_npy/S315_181_prot.npy')
    all_label = np.lib.format.open_memmap('../features_npy/S315_181_label.npy')
